...
/**
 * FormGet function getMaxlength 
 * function used for getting maxlength attribute for element
 */
function getMaxLength(container_li_id) {
	if (document.getElementById('text_element' + container_li_id).hasAttribute('maxlength')) {
		return document.getElementById('text_element' + container_li_id).getAttribute('maxlength');
	}
	return undefined;
}
...
//    shadow_hide();
	...
	if (field_type == 'If_Question')
    {
		...
		form_get_span.innerHTML = "<input id='conditionCheckbox' type='checkbox' onclick='buildConditionLogic(this)'><label>Apply Conditional Logic</label><br/>" +
                "<input style='display:none' id='setting_show_condition' type='button' onclick='showConditionDialog(this);' value='View Conditional Logic'/>";
		
        field_setting_sub.appendChild(form_get_span);
	}
	
	var current_value = getMaxLength(container_li_id);
...
//		  var wrapper_span = document.createElement('span');
		...
		var employee_label = '<label>Employee</label>';
		var employee_select = '<select class="form-control" name="confirm_with" onchange="setMaxlength(this.value,' + container_li_id + ')">';
		for (var id in all_employees.contacts) {
			var selected = "";
			if (current_value !== undefined && id == current_value) {
				selected = 'selected="true"';
			}
			employee_select += '<option value="' + id + '" ' + selected + '>' + all_employees.contacts[id].name + '</option>';
		}
		employee_select += '</select>';
		form_get_span.innerHTML = employee_label + employee_select;
...
//        maxlength_text.setAttribute('onkeyup', 'setMaxlength(this.value,' + container_li_id + ')');
		...
		employee_label = '<label>Notify to Employee</label>';
		var employee_select = '<select class="form-control" name="notify_to" onchange="setMaxlength(this.value,' + container_li_id + ')">';
		for (var id in all_employees.contacts) {
			var selected = "";
			if (current_value !== undefined && id == current_value) {
				selected = 'selected="true"';
...
//        maxlength_text.setAttribute('onkeyup', 'setMaxlength(this.value,' + container_li_id + ')');
		...
		////////
		form_label = '<label>Form to run</label>';
		var form_select = '<select class="form-control" name="notify_to" onchange="setMaxlength(this.value,' + container_li_id + ')">';
		for (var id in all_forms.forms) {
			var selected = "";
			if (current_value !== undefined && id == current_value) {
				selected = 'selected="true"';
			}
			form_select += '<option value="' + id + '" ' + selected + '>' + all_forms.forms[id].title + '</option>';
		}
		form_select += '</select>';
		
		var new_form = '<div><a href="#" onclick="return create_new_form(this);">New form</div>';
		
		form_get_span.innerHTML = form_label + form_select + new_form;
		////////
		
		//form_get_span_required.appendChild(maxlength_text);
        jQuery(".requirred-check").before(form_get_span);
    }
...
window.onbeforeunload = function reload()
{
    ...
    if (element_div.length === 0 || can_close)
    {
        return null;
    }
    else{
